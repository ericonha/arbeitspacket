...
def get_workers_info(filepath, lista_months):
    ...
    for line in pf.values.__array__()[1:]:
        if float(line[6]) != 0:
            w = worker.Worker(line[0], line[7], len(line) - 5, float(line[8])/100, line[1], line[2])
        else:
            w = worker.Worker(line[0], line[7], len(line) - 5, 0, line[1], line[2])
        for i in range(2, len(line) - 3):
            if math.isnan((line[i + 1])):
                line[i + 1] = 10.5
            w.hours_available[i-2] = [line[i + 1]]
...
def get_nrs(df, entity):
    numbers = []
    counter = 0
    index_entity = get_name(df, entity)
    for line in df.values.__array__():
        line_to_string = str(line[1])
        if line_to_string != 'nan': # and line[index_entity] != 0 and not math.isnan(line[index_entity]):
            try:
